...
class Magestore_Customerreward_Model_Actions_Spend extends Magestore_Customerreward_Model_Actions_Abstract
{
	protected $_action = 'spend';
	protected $_title = 'Spend for order %s';
	
	protected function _calculateAmount(){
		$order = $this->getActionObject();
		if ($amount = $order->getPointAmount()){
	...
	public function getTitleHtml($isAdminArea = false){
		if(!$this->getTransaction()->getData('extra_content')){
			return $this->getTransaction()->getTitle();
		}
