...
define("BCD_C128_BAR_4"              ,   4);
  ...
  class Gpf_BarCode_Type_BarCode {
	...
	function DrawSingleBar($xPos, $yPos, $xSize, $ySize) {
	  if ($xPos>=0 && $xPos<=$this->mWidth  && ($xPos+$xSize)<=$this->mWidth &&
 	     ...
 	     throw new Gpf_BarCode_Exception("DrawSingleBar: Out of range");
		 return false;    
	  }					  
	 ...
	 if (($this->mStyle & BCS_BORDER)) {
	   ...
	   if ($this->mStyle & BCS_IMAGE_PNG) {
	     ...
	     ImagePng($this->mImg);
			    ...
			    ImageJpeg($this->mImg);
	   		} else throw new Gpf_BarCode_Exception("FlushObject: No output type");
	 }											
