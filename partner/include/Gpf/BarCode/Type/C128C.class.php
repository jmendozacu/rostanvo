...
*/
  ...
  */
  ...
  class Gpf_BarCode_Type_C128C extends Gpf_BarCode_Type_BarCode {
   var $mCharSet, $mChars;
   function Gpf_BarCode_Type_C128C($Width, $Height, $Style, $Value)
   {
     $this->Gpf_BarCode_Type_BarCode($Width, $Height, $Style);
	 $this->mValue   = $Value;
         $this->mChars   = array
          (
   ...
   function GetSize($xres) {
	 ...
	 if ($len == 0)  {
	   $this->mError = "Null value";
   	   throw new Gpf_BarCode_Exception("GetRealSize: null barcode value");
	   return false;
	   }
	 $ret = 0;
	 ...
	 if (($len%2) != 0) {
            ...
            throw new Gpf_BarCode_Exception("GetSize: failed C128-C requiremente");
		return false;
		}		 
   ...
   function DrawCheckChar($DrawPos, $yPos, $ySize, $xres)
   {
    ...
    function DrawObject ($xres)
    {
	 ...
	 if (($size = $this->GetSize($xres))==0) {
     	throw new Gpf_BarCode_Exception("GetSize: failed");
	    return false;
	 }    
