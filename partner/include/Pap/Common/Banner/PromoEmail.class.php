...
class Pap_Common_Banner_PromoEmail extends Pap_Common_Banner {
    
    protected function getBannerCode(Pap_Common_User $user, $flags, $data1 = '', $data2 = '') {
        $email = $this->getDescription($user);
        
    	$email = $this->replaceUserConstants($email, $user);
    	$email = $this->replaceUrlConstants($email, $user, 0, '', $data1, $data2);	
    	$email = $this->replaceBannerConstants($email, $user);	
